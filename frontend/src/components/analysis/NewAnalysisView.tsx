...
export function NewAnalysisView({ onRunPipeline, onNavigateHome }: NewAnalysisViewProps) {
  ...
  return (
    <div className="relative min-h-screen flex flex-col bg-[#0A0E14] text-gray-100 font-sans selection:bg-cyan-500/30 selection:text-cyan-200 overflow-x-hidden">
      ...
      <header className="relative z-40 w-full bg-[#070A0F]/90 backdrop-blur-md border-b border-[#1F2937] px-4 md:px-6 py-2.5">
        <div className="max-w-[1700px] mx-auto flex items-center justify-between">
          ...
          >
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-cyan-500 to-blue-700 flex items-center justify-center shadow-md shadow-cyan-500/20 border border-cyan-400/40">
              <Radar className="w-4 h-4 text-white animate-spin-slow" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] uppercase font-mono px-1.5 py-0.5 rounded bg-cyan-950/70 text-cyan-400 border border-cyan-700/40 font-semibold">
                  v2.4
                </span>
              </div>
          ...
          {/* Center: Live Station Status Pill */}
          <div className="hidden md:flex items-center gap-2.5 px-3.5 py-1 rounded-full bg-[#0E1522] border border-emerald-500/30 text-xs font-mono">
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75" />
              <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500" />
            </span>
            <span className="text-emerald-400 font-semibold tracking-wide">
              LIVE — SHAR SRIHARIKOTA (ISTRAC)
            </span>
          </div>

          {/* Right: Operational Controls & Identity Chip */}
          <div className="flex items-center gap-2.5 text-xs">
            {/* Live Mode Toggle */}
            <button
              onClick={() => setIsLiveMode(!isLiveMode)}
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border font-mono transition-all cursor-pointer ${
                isLiveMode
                  ? "bg-emerald-950/40 border-emerald-600/50 text-emerald-300"
                  : "bg-gray-900/60 border-gray-700 text-gray-400"
              }`}
            >
              <Radio className={`w-3.5 h-3.5 ${isLiveMode ? "text-emerald-400 animate-pulse" : "text-gray-500"}`} />
              <span className="text-[11px] font-semibold">
                {isLiveMode ? "LIVE FEED ACTIVE" : "SIMULATION"}
              </span>
            </button>

            {/* Theme Toggle (Visual) */}
            <div className="hidden sm:flex items-center p-1.5 rounded-lg bg-[#111827] border border-[#1F2937] text-gray-400 hover:text-white cursor-pointer">
              <Moon className="w-3.5 h-3.5" />
            </div>

            {/* Operator Identity Chip */}
            <div className="flex items-center gap-2 px-2.5 py-1 rounded-lg bg-[#111827] border border-[#1F2937]">
              <div className="w-5 h-5 rounded-full bg-gradient-to-tr from-amber-500 to-red-500 flex items-center justify-center text-[10px] font-bold text-black">
                IS
              </div>
              <div className="font-mono text-left">
                <p className="text-[11px] text-gray-200 font-semibold leading-none">ISRO / L-3 SCI</p>
                <p className="text-[9px] text-gray-500 leading-none mt-0.5">SEC-CLEAR: TIER-1</p>
              </div>
            </div>
            ...
            <button
              ...
              className="p-1.5 rounded-lg bg-[#111827] border border-[#1F2937] text-gray-400 hover:text-red-400 hover:border-red-500/40 transition-colors cursor-pointer"
            >
      ...
      <div className="relative z-10 flex-1 flex">
        ...
        <aside
          className={`relative z-30 bg-[#070A0F]/80 backdrop-blur-sm border-r border-[#1F2937] transition-all duration-300 flex flex-col justify-between ${
        ...
        >
          ...
          <div className="p-3 space-y-1 font-mono text-xs">
            {[
              { id: "dash", label: "Dashboard", icon: LayoutDashboard, active: false, action: onNavigateHome },
              { id: "new", label: "New Analysis", icon: PlusCircle, active: true },
              { id: "history", label: "Analysis History", icon: History, active: false },
              { id: "profile", label: "Operator Profile", icon: User, active: false },
              { id: "eval", label: "Benchmark & Eval", icon: BarChart3, active: false },
              { id: "registry", label: "Specialist Registry", icon: Cpu, active: false },
            ].map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left transition-all cursor-pointer ${
                    item.active
                      ? "bg-cyan-950/40 text-cyan-300 border-l-2 border-cyan-400 font-semibold shadow-inner"
                      : "text-gray-400 hover:text-gray-200 hover:bg-gray-800/40"
                  }`}
                >
                  <Icon
                    className={`w-4 h-4 shrink-0 ${item.active ? "text-cyan-400" : "text-gray-400"}`}
                  />
          ...
          {/* Bottom Pinned Telemetry Card & Collapse Button */}
          <div className="p-3 border-t border-[#1F2937]">
            {!sidebarCollapsed && (
              <div className="mb-3 p-2.5 rounded-lg bg-[#0E1522] border border-[#1F2937] text-[11px] font-mono">
                <div className="flex items-center justify-between">
                  <span className="text-gray-400">Telemetry Stream</span>
                  <span className="px-1.5 py-0.2 rounded bg-emerald-950/60 text-emerald-400 border border-emerald-500/30 text-[9px] font-bold">
                    SYNCED
                  </span>
                </div>
                <p className="text-[10px] text-gray-500 mt-1">NODE // ISRO-IN-09</p>
                <div className="mt-1.5 w-full bg-gray-800 h-1 rounded-full overflow-hidden">
                  <div className="bg-cyan-400 h-full w-[84%] animate-pulse" />
                </div>
              </div>
            )}
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-1.5 text-gray-500 hover:text-gray-300 rounded bg-[#111827] border border-[#1F2937] text-xs font-mono transition-colors cursor-pointer"
            >
              {sidebarCollapsed ? (
